...
protected:
	...
	virtual basic_item* allocateSpecificItem()=0;

	// remove item from heap for the specific type  (calling the porper distructor)
	virtual void deallocateSpecificItem(basic_item* item)=0;

	// this deletes the entire array and all the elements in it
	void deallocateArrayAndContent()
	{
...
public: 
	general_array(){max_arraysize=0; memallocated=false; tot_items=0; thearray=NULL; resetCurrIndexDefault();}	
...
class int_array : public general_array{
protected:
	
	// allocate item on heap for the specific type (calling the porper constructor)
	virtual basic_item* allocateSpecificItem()
	{
		integer_item* emptyitem=new integer_item;		
		return emptyitem;
	}
	...
	// remove item from heap for the specific type  (calling the porper distructor)
	virtual void deallocateSpecificItem(basic_item* basic_ptr)
	{
		integer_item* emptyitem=NULL;
		integer_item* itemToBeDeleted=typecastItem(basic_ptr, emptyitem);
		
		// now calling delete will trigger the destructor of the derived class
...
public:
	int_array(){;}
	~int_array(){deallocateArrayAndContent();}
